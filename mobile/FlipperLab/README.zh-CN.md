...
设备页新增蓝牙远程屏幕与六键入口；通信方式、覆盖范围和真机验收项见 [蓝牙远程操作说明](REMOTE_CONTROL.zh-CN.md)。此功能新增后的编译和真机结果以本次变更的 CI 与验收记录为准，不能沿用下表旧版 CI 的结论。
...
- 连接后可打开远程操作页：屏幕帧来自 Flipper 固件，手机六键默认发送短按事件，也可对下一键发送一次长按事件；离开页面停止屏幕流。远程操作不改变 Flipper 原有硬件能力，真机尚未验收。
