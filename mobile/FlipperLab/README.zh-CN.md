...
- 中文功能条目由本仓库固件中的应用清单核对，点击后向 Flipper 发送应用启动请求；连接未就绪时不可执行。
- 可从 Flipper 的 `/ext/apps` 读取已安装的其他 `.fap` 应用，直接在手机功能页选中启动；设备需要 SD 卡且该应用确实存在。
- 若设备上已有应用运行，固件加载器会拒绝新启动。手机显示失败原因，先在 Flipper 退出当前应用后重试。
